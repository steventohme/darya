...
pub fn render(frame: &mut Frame, area: Rect, app: &mut App, is_focused: bool) {
    ...
    let Some(ref mut note) = app.note else {
        // No note loaded — render placeholder
    ...
    let mode_label = if note.read_only { "VIEW" } else { "EDIT" };
    let title = format!(
        " Notes — {} {}{} ",
        note.worktree_name, mode_label, modified_indicator
    );
    ...
    // Build syntax highlighter for markdown
    let syntax_hl = {
